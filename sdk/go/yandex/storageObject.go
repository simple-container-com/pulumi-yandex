...
// - `content` (String). Literal string value to use as the object content, which will be uploaded as UTF-8-encoded text. Conflicts with `source` and `contentBase64`.
// - `contentBase64` (String). Base64-encoded data that will be decoded and uploaded as raw bytes for the object content. This allows safely uploading non-UTF8 binary data, but is recommended only for small content such as the result of the `gzipbase64` function with small text strings. For larger objects, use `source` to stream the content from a disk file. Conflicts with `source` and `content`.
// - `contentType` (String). A standard MIME type describing the format of the object data, e.g. `application/octet-stream`. All Valid MIME Types are valid for this input.
// - `id` (String).
// - `key` (**Required**)(String). The name of the object once it is in the bucket.
// - `objectLockLegalHoldStatus` (String). Specifies a [legal hold status](https://yandex.cloud/docs/storage/concepts/object-lock#types) of an object. Requires `objectLockConfiguration` to be enabled on a bucket.
// - `objectLockMode` (String). Specifies a type of object lock. One of `["GOVERNANCE", "COMPLIANCE"]`. It must be set simultaneously with `objectLockRetainUntilDate`. Requires `objectLockConfiguration` to be enabled on a bucket.
// - `objectLockRetainUntilDate` (String). Specifies date and time in RTC3339 format until which an object is to be locked. It must be set simultaneously with `objectLockMode`. Requires `objectLockConfiguration` to be enabled on a bucket.
...
// - `source` (String). The path to a file that will be read and uploaded as raw bytes for the object content. Conflicts with `content` and `contentBase64`.
// - `sourceHash` (String). Used to trigger object update when the source content changes. So the only meaningful value is `filemd5("path/to/source"). The value is only stored in state and not saved by Yandex Storage.
