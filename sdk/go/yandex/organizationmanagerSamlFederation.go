...
//
// - `autoCreateAccountOnLogin` (Bool). Add new users automatically on successful authentication. The user will get the `resource-manager.clouds.member` role automatically, but you need to grant other roles to them. If the value is `false`, users who aren't added to the cloud can't log in, even if they have authenticated on your server.
// - `caseInsensitiveNameIds` (Bool). Use case-insensitive name IDs.
// - `cookieMaxAge` (String). The lifetime of a Browser cookie in seconds. If the cookie is still valid, the management console authenticates the user immediately and redirects them to the home page. The default value is `8h`.
...
// - `organizationId` (**Required**)(String). The organization to attach this SAML Federation to.
// - `ssoBinding` (**Required**)(String). Single sign-on endpoint binding type. Most Identity Providers support the `POST` binding type. SAML Binding is a mapping of a SAML protocol message onto standard messaging formats and/or communications protocols.
// - `ssoUrl` (**Required**)(String). Single sign-on (SSO) endpoint URL. Specify the link to the IdP login page here.
// - `securitySettings` [Block]. Federation security settings.
//   - `encryptedAssertions` (Bool). Enable encrypted assertions.
//   - `forceAuthn` (Bool). Force authentication on session expiration
//
// - `timeouts` [Block].
//   - `create` (String).
//   - `delete` (String).
