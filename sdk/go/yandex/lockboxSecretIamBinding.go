...
//
// > **Warning:** This resource is authoritative for the given `role` on the target `secret` and manages the complete set of its members. When you change or delete `LockboxSecretIamBinding`, the `role` may be removed from other subjects on the `secret` as well — including subjects granted outside of this resource (via the corresponding `*_iam_member` resource, the management console, CLI or API). Those subjects are not tracked in the Terraform state, so a plain `pulumi preview` does not list them. Be careful.
//
// ## Example Usage
//
// ```go
...
//
// - `role` (**Required**)(String). The role that should be assigned. Only one LockboxSecretIamBinding can be used per role.
...
//
// ```sh
// $ pulumi import yandex:index/lockboxSecretIamBinding:LockboxSecretIamBinding viewer "abjjf**********p3gp8,viewer"
// ```
type LockboxSecretIamBinding struct {
	pulumi.CustomResourceState

	// The ID of this resource.
	LockboxSecretIamBindingId pulumi.StringOutput `pulumi:"lockboxSecretIamBindingId"`
	...
	// The role that should be assigned. Only one LockboxSecretIamBinding can be used per role.
...
type lockboxSecretIamBindingState struct {
	// The ID of this resource.
	LockboxSecretIamBindingId *string `pulumi:"lockboxSecretIamBindingId"`
	// An array of identities that will be granted the privilege in the `role`. Each entry can have one of the following values:
	//  * **userAccount:{user_id}**: A unique user ID that represents a specific Yandex account.
	//  * **serviceAccount:{service_account_id}**: A unique service account ID.
	//  * **federatedUser:{federated_user_id}**: A unique federated user ID.
...
type LockboxSecretIamBindingState struct {
	// The ID of this resource.
	LockboxSecretIamBindingId pulumi.StringPtrInput
	// An array of identities that will be granted the privilege in the `role`. Each entry can have one of the following values:
	//  * **userAccount:{user_id}**: A unique user ID that represents a specific Yandex account.
	//  * **serviceAccount:{service_account_id}**: A unique service account ID.
	//  * **federatedUser:{federated_user_id}**: A unique federated user ID.
...
// The ID of this resource.
func (o LockboxSecretIamBindingOutput) LockboxSecretIamBindingId() pulumi.StringOutput {
	return o.ApplyT(func(v *LockboxSecretIamBinding) pulumi.StringOutput { return v.LockboxSecretIamBindingId }).(pulumi.StringOutput)
}
